...
```
idle ──[点击开始]──→ collecting ──[collection_complete]──→ complete
  ↑                     │                                    │
  │                [点击停止]                          [点击重新采集]
  │                     │                                    │
  └─────────────────────┘                                    │
...
**注意**: 完成触发来自 content script 的 `collection_complete` 消息（非 `hasMore=false`），因为主列表结束后仍需展开回复。
...
### 进度显示
- `total` = `totalComments + totalRepliesExpected`（近似值）
- `displayTotal` = 完成时取 `max(total, collected)`，防止分母 < 分子
- 完成状态强制 `pct = 100`，避免因回复估计偏差导致 UI 不满格
- 采集中正常计算百分比，用 `Math.min(100, ...)` 兜底
